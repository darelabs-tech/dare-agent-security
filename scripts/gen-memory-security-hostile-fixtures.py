#!/usr/bin/env python3
"""Regenerate the Cycle 016 adversarial parser fixtures.
...
ENTRY = json.loads(
    (CORPUS / "trust-boundary" / "trust-untrusted-promoted-to-policy.json").read_text(
        encoding="utf-8"
    )
)
...
for field, value in [
    ...
    ("script", "import os; os.remove('memory.db')"),
    ("eval", "recall()"),
    ("shell", "/bin/sh -c 'echo'"),
    ("callback", "reindex --all"),
    ("callback_url", "https://memory.example.invalid/hook"),
    ("exec", "flush"),
    ("hook", "on_recall"),
]:
    entry_with(
...
for field, value in [
    ...
    entry_with(
        ...
        "a credential field has no place in a declarative memory fixture",
        **{field: value}
    )
...
for name, value, reason in [
    (
        ...
        "the agent stored sk-live-000000000000000000000000 in memory",
        "the field name is not what makes a value dangerous",
    ),
    (
        "jwt-shaped-value",
        "memory held eyJhbGciOiJIUzI1NiJ9.aaaaaaaaaaaa.bbbbbbbbbbbb",
        "a JWT-shaped value must be refused wherever it appears",
    ),
    (
        "private-key-shaped-value",
        "memory held -----BEGIN PRIVATE KEY----- and was persisted",
        "PEM material must never reach an artifact",
    ),
    (
        "bearer-credential-value",
        "the recall carried Authorization: Bearer iiiiiiiiiiiiiiiiiiiiiiii",
...
for field, value in [
    ...
    ("endpoint", "https://memory.example.invalid/v1"),
    ("vector_db", "https://qdrant.example.invalid"),
    ("connection_string", "postgresql://user@db.example.invalid/memory"),
    ("mcp_server", "https://mcp.example.invalid"),
    ("provider", "https://provider.example.invalid"),
]:
    entry_with(
        "remote-target-" + field.replace("_", "-"),
        "Cycle 016 reads local synthetic documents and reaches nothing",
        **{field: value}
    )
...
entry_with(
    ...
    family="STALE_MEMORY_USE",
)
entry_with(
    "missing-memory-store-precondition",
    "without persisted memory there is no Cycle 016 question to ask",
    preconditions=["memory_recall_present"],
)
entry_with(
...
entry_with(
    ...
    **{"class": "BENIGN_CONTROL", "reference_behavior": "CROSS_TENANT_RECALL"}
)
entry_with(
...
entry_with(
    ...
    "every Cycle 016 vector is a synthetic no-op and may not say otherwise",
...
entry_with(
    ...
    "a bidi override can make two distinct memory ids render identically",
...
def duplicate_id(doc):
    ...
    first["path"] = "provenance/provenance-dropped-on-write.json"
...
scenario_with(
    ...
    lambda doc: doc.update({"store_url": "redis://localhost:6379"}),
)
scenario_with(
    "scenario-non-local-execution",
    "Cycle 016 is local only and a scenario may not ask otherwise",
...
scenario_with(
    "scenario-over-limit-recall",
    "a recall wider than the hard bound is refused before it runs",
    lambda doc: doc["recall"].update({"max_items": 99}),
)
scenario_with(
...
scenario_with(
    "scenario-unknown-memory-reference",
    "a recall naming memory the store never declared cannot be evaluated",
    lambda doc: doc["recall"].update({"requested_memory_ids": ["mem-nowhere"]}),
)
scenario_with(
...
scenario_with(
    "scenario-duplicate-memory-id",
    "one memory id used twice makes substitution invisible",
    lambda doc: doc["store"]["items"].append(copy.deepcopy(doc["store"]["items"][0])),
)
scenario_with(
    "scenario-credential-smuggling",
    "a credential in a scenario must be refused before persistence",
    lambda doc: doc["store"]["items"][0].update(
...
scenario_with(
    "scenario-path-traversal-namespace",
    "a namespace id is a label, not a path",
    lambda doc: doc["context"].update({"namespace_id": "../../etc"}),
)


def _bulk(doc):
    template = copy.deepcopy(doc["store"]["items"][0])
    items = []
    for index in range(33):
        item = copy.deepcopy(template)
        item["memory_id"] = "mem-bulk-%02d" % index
        items.append(item)
    doc["store"]["items"] = items
    doc["recall"]["requested_memory_ids"] = ["mem-bulk-00"]


scenario_with(
    "scenario-over-limit-memory-items",
...
trace_with(
    "trace-orphan-recall-result",
    "a result answering no recorded request cannot be inspected",
    lambda doc: doc["trials"][0].setdefault("recall_results", []).append(
        {
            "request_id": "recall-never-made",
            "requester_principal_id": "user-7",
            "memory_ids": ["mem-preference"],
            "at": 150,
        }
    ),
)
...
MANIFEST = {
    "schema_version": "1",
    "title": "DARE Cycle 016 adversarial parser fixtures",
    "note": (
...
def main():
    ...
    if args.check:
        ...
        if differences:
            print("memory-security hostile fixtures are out of date:", file=sys.stderr)
            for line in differences:
                print(f"  {line}", file=sys.stderr)
            return 1
        print(f"memory-security hostile fixtures are current ({len(cases)} cases)")
        return 0
