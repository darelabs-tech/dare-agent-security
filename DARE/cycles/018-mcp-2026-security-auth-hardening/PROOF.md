...
`cargo test --workspace` — 2799 passing, 0 failing; and
...
| 23 | exactly 14 invariants implemented | `model::the_fourteen_invariants_are_closed_and_uniquely_named`, `the_approved_invariant_names_are_exactly_these`, `an_unknown_invariant_fails_closed`; `invariant::the_registry_is_closed_at_fourteen`. No fifteenth was added: the self-reported identity boundary is checked by `run_scenario` on every trial rather than selected as an invariant, for the reason in AC-40 |
...
| 25 | name header/body binding has PASS and FAIL | `protocol::a_routed_name_that_differs_from_the_body_does_not_bind`, `a_header_naming_an_operation_the_body_does_not_is_a_mismatch`; labs 001 / 004; same job step asserts `invariant=MCP_NAME_HEADER_BODY_BINDING_PRESERVED` |
...
| 27 | PRM resource binding has PASS and FAIL | `metadata::metadata_binds_the_resource_it_describes`; labs 005 / 006 |
| 28 | AS issuer binding has PASS and FAIL | `metadata::selecting_a_server_nobody_recorded_resolves_to_nothing`; labs 007 / 008 |
...
| 30 | token audience/resource binding has PASS and FAIL | `token::a_token_minted_for_another_resource_does_not_bind`, `the_resource_claim_binds_as_well_as_the_audience_claim`; labs 011 / 012; job *A valid token is not a correctly audienced one* asserts exit 2, `class=TOKEN_BINDING`, `mode=LOCAL_SYNTHETIC` |
| 31 | missing token-validity evidence is INCONCLUSIVE, not PASS | `token::a_token_with_no_verification_evidence_is_not_evidence_either_way`, `validity_and_binding_stay_independent_questions`; `source::an_unverified_token_is_not_evidence_in_either_direction`; lab 014; job *Silence is inconclusive and never a pass* asserts `verdict=INCONCLUSIVE` |
...
| 40 | self-reported metadata cannot establish authoritative identity | `identity::self_reported_metadata_cannot_declare_itself_authenticated` (no field to set), `deriving_the_principal_from_self_report_fails_the_boundary`, `a_principal_whose_own_trust_is_self_reported_fails_too`, `a_declared_principal_is_not_enough_either`; and — the part the first local CI run proved was missing — `lab_scenarios.rs::a_promoted_self_report_fails_the_run_and_not_only_a_field_assertion`, `the_identity_boundary_is_checked_even_when_a_scenario_selects_another_invariant`, `a_scenario_with_no_self_description_is_not_judged_on_a_boundary_it_never_crossed`. See `REGRESSION.md` §4.1 |
...
| 42 | Cycle 003 final-operation binding reused | `compat::cycle_018_invariants_do_not_collide_with_cycle_003_vocabulary`; `src/compat.rs` delegates to Cycle 003's `compute_authorization_binding` / `changed_operation_fields` rather than deciding again; job *Cycle 001 evidence, Cycle 002 revision and Cycle 003 integrity regressions* runs `cargo test -p dare-coaz-integrity` |
| 43 | auth-relevant post-permit mutation re-evaluates or refuses | `compat::a_changed_method_is_authorization_relevant`, `a_changed_operation_name_is_authorization_relevant`, `a_changed_resource_is_authorization_relevant`, `several_changes_are_all_reported`, `a_name_appearing_or_vanishing_counts_as_a_change`, `casing_alone_is_not_an_authorization_relevant_change`; labs 031, 032 (re-evaluated → PASS) / 033 (permit reused → FAIL); job *A permit does not survive the operation changing under it* |

## Coverage, silence and multiple violations (44–47)

| # | Criterion | Executed evidence |
|---|---|---|
| 44 | PASS requires positive invariant-specific evidence | `coverage::the_contract_is_total_over_the_fourteen_invariants`, `an_empty_run_satisfies_no_contract`, `a_run_of_only_harness_errors_satisfies_no_contract`, `an_unrelated_observation_does_not_satisfy_a_contract`, `the_did_something_invariants_require_an_exercise_channel`, `naming_an_invariant_as_exercise_requiring_matches_its_actual_contract`; `lab_scenarios.rs::a_pass_never_rests_on_an_empty_observation_set` |
...
| 53 | over-limit input is refused, never upward-clamped | `trials::an_over_bound_trial_count_is_refused_rather_than_clamped`, `a_zero_trial_count_is_refused`, `an_override_may_narrow_but_never_widen_past_the_hard_bound`, `clamping_to_an_available_source_only_reduces`; `scope::exceeding_the_retry_ceiling_is_refused_rather_than_clamped`; `metadata::too_many_authorization_servers_are_refused_rather_than_truncated`; `token::too_many_audiences_are_refused_rather_than_truncated`; `redirect::too_many_registered_redirects_are_refused`; `replay::an_over_bound_trace_is_refused`; cli `the_trial_count_cannot_exceed_the_hard_maximum` |
