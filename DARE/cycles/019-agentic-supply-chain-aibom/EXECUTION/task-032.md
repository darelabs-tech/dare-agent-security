...
`a_model_with_no_digest_fails_completeness_and_a_framework_does_not` asserts both halves: the model with no digest is a violation, and the framework beside it is not. A framework describes an arrangement rather than bytes; a digest requirement against it is unremediable.
