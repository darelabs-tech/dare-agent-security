...
//!
//! No second executor is introduced. Observations still come from the Cycle 016
...
//! synthetic one. Memory a scenario describes as poisoned is *described*, never
//! written anywhere — the boundary crossing is proven from declarations rather
//! than by touching a store.
//!
//! The step's arguments are identifiers only: scenario, acting principal,
//! tenant, namespace, trial index. No memory content, no provenance detail and
//! no policy values travel into the control substrate.
//!
...
/// Cycle 009 budget for a Cycle 016 local-synthetic run.
///
/// State changes, bytes written and external egress are all zero, so an attempt
/// to mutate a store or exfiltrate memory is refused by the Cycle 009 policy
/// rather than by a Cycle 016 rule of its own.
pub fn synthetic_budget(trials: u32) -> ExecutionBudget {
    ExecutionBudget {
        schema_version: "1".to_owned(),
        id: "budget-memory-security-local-synthetic".to_owned(),
        ...
        max_retries: 0,
        max_chain_depth: crate::limits::HARD_MAX_NAMESPACES,
    }
}
...
pub struct MemoryControlSnapshot {
    /// Kill-switch state after the last inspected step.
    pub kill_switch: String,
    /// Operations consumed against the Cycle 009 budget.
    pub operations: u32,
    /// State changes observed. Always zero for Cycle 016.
    pub state_changes: u32,
    /// External egress bytes observed. Always zero for Cycle 016.
...
pub struct LocalSyntheticAdapter {
    ...
    control: RefCell<MemoryControlSnapshot>,
}
...
impl LocalSyntheticAdapter {
    ...
    pub fn new(target_id: impl Into<String>, trials: u32) -> Self {
        Self {
            ...
            control: RefCell::new(MemoryControlSnapshot {
    ...
    pub fn for_scenario(scenario: &MemorySecurityScenario, trials: u32) -> Self {
    ...
    pub fn control_snapshot(&self) -> MemoryControlSnapshot {
    ...
    fn observe(&self, request: &TrialRequest<'_>) -> Result<RawTrialOutput> {
        ...
        // Cycle 009 budget next: refuse the operation that would cross a bound.
        {
            ...
            if let Err(error) = state.check_next(&step, &self.budget) {
                ...
                return Err(MemorySecurityError::BudgetExhausted(error.to_string()));
            }
            state.consume(&step);
...
mod tests {
    ...
    #[test]
    fn the_step_carries_identifiers_and_no_memory_content() {
        let scenario = scenario();
        let adapter = adapter();
        let step = adapter.step_for(&TrialRequest {
            trial_index: 0,
        ...
        // Identifiers must be present, so the step is not silently empty.
        assert!(text.contains("user-7"));
        assert!(text.contains("tenant-a"));
        assert!(text.contains("ns-support"));
    }
    ...
    #[test]
    fn a_step_that_would_change_state_is_killed() {
    ...
    fn pointing_an_approved_run_at_another_scenario_trips_the_kill_switch() {
        ...
        let scenario = scenario();
        let adapter = LocalSyntheticAdapter::new("MEMORY-LAB-024", 3);
        let raw = adapter
            .observe(&TrialRequest {
                trial_index: 0,
                scenario: &scenario,
        ...
        assert!(raw.recall_results.is_empty());
    ...
    #[test]
    fn poisoned_memory_is_described_and_never_written_anywhere() {
        let mut scenario = scenario();
        scenario
            .lab
            .as_mut()
            .expect("the fixture declares a lab spec")
            .reference_behavior = ReferenceBehavior::ContentSubstituted;
        ...
        // The substitution is observed and reported, not performed.
        assert_eq!(
            evaluate(
