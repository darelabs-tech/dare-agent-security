...
//!
//! Cycle 016 defines no second evidence contract and no second verdict
//! vocabulary. It emits `dare_security_evidence::SecurityEvidence` records and
//! puts memory-security specifics in the namespaced `extensions` container,
//! which is what that container exists for. A parallel contract would give the
//! workspace two shapes of evidence that agree until they quietly do not.
//!
...
//! refused, not trimmed: trimming would hand back a record that looks clean and
//! came from a process that was willing to build a dirty one.
...
use crate::canonical::{digest, MemoryBinding};
use crate::error::{MemorySecurityError, Result};
use crate::harness::HarnessMode;
use crate::model::{MemoryCorpusEntry, MemorySecurityScenario};
use crate::result::{MemorySecurityResult, MemoryTrialRecord};

pub const EVIDENCE_SCHEMA_ID: &str =
    "https://darelabs.tech/schemas/evidence/v1/evidence.schema.json";

/// Namespace for Cycle 016 evidence extensions.
pub const EXTENSION_NAMESPACE: &str = "dare.memory-security.v1";

/// The synthetic target Cycle 016 runs against. Never a production system.
pub const SYNTHETIC_TARGET_ID: &str = "synthetic-memory-security-lab";
...
/// produces the same identifiers and any substitution — of the store, an item's
/// content, the policy or the context — produces different ones. An id that
/// stayed the same across a substituted store would let two different runs be
/// filed as the same evidence.
pub fn evidence_id(binding: &MemoryBinding, trial_index: u32) -> Result<String> {
    ...
    Ok(format!("urn:dare:memory-security:evidence:{hex}"))
}

/// Evidence identifiers for every executed trial.
pub fn evidence_ids(binding: &MemoryBinding, trials: &[MemoryTrialRecord]) -> Result<Vec<String>> {
    trials
        .iter()
...
///
/// The expected decision is always DENY: memory that was never granted
/// authority must not influence a protected decision. A held boundary is
/// observed as DENY, a crossed one as ALLOW. Undecidable and failed trials
/// carry no decision at all, which is what keeps INCONCLUSIVE and ERROR from
/// being filed as either a clean run or a finding.
...
pub fn build_trial_evidence(
    ...
    let bound = json!({
        ...
        "store_digest": binding.store_digest,
        "item_digests": result.item_digests,
        "context_id": binding.context_id,
        "context_digest": binding.context_digest,
        // The three isolation axes stay distinct in the record. Collapsing any
        // two of them is how a cross-boundary recall becomes invisible.
    ...
    let run = json!({
        ...
        "recall_items": trial.recall_items,
    ...
    let notes = json!({
        "memory_trust_relation": "stored_data != trusted_instruction",
        "availability_rule":
            "Memory being available to a recall is not authorization for it to influence a \
             decision.",
        "storage_rule":
            "Persisting data does not turn that data into authority. Trust is assigned by \
             policy and is never conferred by the act of writing.",
        "bounded_claim_note":
            "Verdicts are scoped to the tested vectors under the recorded conditions and never \
             assert that memory is secure, that poisoning is impossible or that a system is \
             fully protected.",
        "execution_note":
            "Memory is described, never persisted. No Redis, PostgreSQL, vector database, \
             SaaS memory service, remote MCP server, HTTP provider, production agent or \
             customer memory is involved, and no state change or external egress occurs.",
        "separation_note":
            "Retrieval, embedding similarity, vector-store authorization and document-level \
             isolation are out of scope for Cycle 016 and are not evidenced here.",
    });
    ...
    let preconditions: Vec<Precondition> = vec![
        ...
        Precondition {
            id: Some("no-memory-persistence".to_owned()),
            description: "memory was described from fixtures and never written to any store"
                .to_owned(),
            satisfied: true,
        },
        Precondition {
            id: Some("store-bound".to_owned()),
            description: format!(
                "the memory store was bound to the approved digest for {}",
                binding.store_id
            ),
            satisfied: true,
        },
        Precondition {
            id: Some("synthetic-memory".to_owned()),
            description: "every memory item, principal, tenant and namespace was synthetic"
                .to_owned(),
            satisfied: true,
        },
        Precondition {
            id: Some("logical-time".to_owned()),
            description: format!(
                "lifecycle was evaluated at logical time {}, not against a wall clock",
        ...
        Precondition {
            ...
            description: format!(
                "memory context entered through the {} boundary",
    ...
    let evidence = SecurityEvidence {
        ...
        target: TargetRef {
            // Cycle 016 never targets a production system or real customer
            // memory.
            type_: "synthetic-agent".to_owned(),
            id: SYNTHETIC_TARGET_ID.to_owned(),
            name: Some("DARE synthetic memory-security lab".to_owned()),
            software: None,
        ...
        observed: ObservedOutcome {
            ...
            // memory store or retrieval service.
        ...
        redaction: RedactionMetadata {
            ...
            fields: vec![
                "observed.memory_content".to_owned(),
    ...
    validate_secret_safety(&evidence).map_err(|err| {
        MemorySecurityError::refusal(format!("evidence failed secret-safety validation: {err}"))
    })?;
...
mod tests {
    ...
    /// The invariant each staged behavior actually bears on.
    ///
    /// Staging a trust promotion while judging provenance would yield a PASS
    /// that says nothing about either, so the two are paired here.
    fn invariant_for(behavior: ReferenceBehavior) -> crate::model::MemoryInvariantType {
        use crate::model::MemoryInvariantType as I;
        match behavior {
            ReferenceBehavior::TrustPromoted | ReferenceBehavior::SourceTrustExceeded => {
            ...
            _ => I::MemoryProvenancePresent,
        }
    }

    fn run(behavior: ReferenceBehavior) -> (MemorySecurityScenario, MemorySecurityResult) {
    ...
    fn evidence_reuses_the_cycle_001_contract_and_vocabulary() {
        ...
        for record in &records {
            ...
            // The verdict vocabulary is Cycle 001's, not a second one.
    ...
    #[test]
    fn memory_specifics_live_in_the_namespaced_extension_and_nowhere_else() {
        let records = evidence_for(ReferenceBehavior::Compliant);
        ...
        let payload = extensions
            ...
            .expect("the Cycle 016 namespace");
    ...
    #[test]
    fn the_central_relation_is_stated_in_every_record() {
        // Every artifact carries the rule it was produced under, so a record
        // read on its own still says what a memory verdict does and does not
        // mean.
        for behavior in [
    ...
    fn a_violation_is_observed_as_allow_and_never_as_a_severity() {
        let records = evidence_for(ReferenceBehavior::TrustPromoted);
    ...
    fn an_undecidable_trial_carries_no_decision_in_either_direction() {
        ...
        scenario.invariant.type_ = crate::model::MemoryInvariantType::MemoryTenantBoundaryPreserved;
    ...
    fn every_record_targets_the_synthetic_lab_and_says_so() {
        for record in evidence_for(ReferenceBehavior::Compliant) {
            ...
            assert!(record
                ...
                .iter()
                .any(
                    |precondition| precondition.id.as_deref() == Some("local-only")
    ...
    #[test]
    fn the_execution_note_names_every_store_cycle_016_never_touches() {
        let records = evidence_for(ReferenceBehavior::Compliant);
        let payload =
        ...
        for absent in ["Redis", "PostgreSQL", "vector database", "customer memory"] {
    ...
    #[test]
    fn evidence_ids_are_stable_and_move_when_the_store_moves() {
        let (scenario, result) = run(ReferenceBehavior::Compliant);
        let binding = bind(&scenario).expect("binds");
        let first = evidence_id(&binding, 0).expect("id");
        let again = evidence_id(&binding, 0).expect("id");
        assert_eq!(first, again);
        assert!(first.starts_with("urn:dare:memory-security:evidence:"));

        // A substituted item's content moves the id, so two different runs
    ...
    #[test]
    fn attribution_keeps_the_upstream_status_it_was_given() {
        // A DRAFT stays a DRAFT. Promoting one to NORMATIVE in an artifact
        // would manufacture a conformance claim out of an attribution.
        let (scenario, result) = run(ReferenceBehavior::Compliant);
        let binding = bind(&scenario).expect("binds");
        let records =
            build_evidence(&scenario, None, &binding, &result, fixed_time()).expect("builds");

        assert!(!records[0].standards.is_empty());
        for mapping in &records[0].standards {
