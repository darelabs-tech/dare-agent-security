//! Cycle 016 adversarial parser fixtures.
//!
...
use dare_memory_security::corpus::{
    builtin_corpus_root, validate_corpus_entry, validate_corpus_registry,
};
use dare_memory_security::error::MemorySecurityError;
use dare_memory_security::model::MemorySecurityScenario;
use dare_memory_security::replay::parse_trace;
...
fn refusal_for(case: &HostileCase, value: &Value) -> Result<(), MemorySecurityError> {
    match case.kind.as_str() {
        ...
        "SCENARIO" => {
            ...
            let scenario: MemorySecurityScenario = serde_json::from_value(value.clone())?;
            scenario.validate()
        }
...
fn a_refusal_is_never_a_security_verdict() {
    // Refusing to run is not evidence that a memory boundary was crossed. No
...
fn a_refusal_never_echoes_a_remote_target() {
    ...
    for case in manifest().cases {
        ...
        let text = error.to_string().to_ascii_lowercase();
        for marker in ["example.invalid", "redis://", "postgresql://"] {
            assert!(
                !text.contains(marker),
                "{} echoed `{marker}` in `{text}`",
                case.id
...
fn the_shipped_corpus_still_loads_with_the_hostile_fixtures_present() {
    ...
    let corpus = dare_memory_security::corpus::builtin_corpus().expect("corpus loads");
