    ...
    pub(crate) fn scenario() -> McpAuthScenario {
        McpAuthScenario {
            ...
            final_operation: FinalOperationContext {
                ...
                performed_arguments: None,
                authorized_principal: Some("user-7".to_owned()),
                performed_principal: Some("user-7".to_owned()),
                authorized_tenant: Some("tenant-a".to_owned()),
                performed_tenant: Some("tenant-a".to_owned()),
                authorized_scopes: vec!["invoices.read".to_owned()],
                performed_scopes: vec!["invoices.read".to_owned()],
